...
router.post('/register',
    ...
    , async (req, res) => {
        ...
        try {
            ...
            const role = email.endsWith("@admin.com") ? "admin" : "employee";

            // in our app only one admin is allowed 
...
router.post("/login",
    ...
    async (req, res) => {
        ...
        try {
            ...
            // console.log(token);
            res.status(200).json({ message: "login successfully", status: true, user: { userId: user._id, name: user.name, role: user.role, taskNumbers: user.taskNumbers} })
        } catch (error) {
